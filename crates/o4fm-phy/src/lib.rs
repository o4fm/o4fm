...
pub fn modulate(frame_bits: &[u8], profile: &RadioProfile) -> Result<Vec<i16>, PhyError> {
    ...
    for sym in 0..symbols {
        ...
        let symbol_idx = usize::from(symbol).min(tones.len().saturating_sub(1));
...
pub fn demodulate(pcm_i16: &[i16], profile: &RadioProfile) -> Result<DemodResult, PhyError> {
    ...
    for sym in 0..symbol_count {
        ...
        let confidence = (best - second).max(1e-6);
        for bit in 0..bits_per_symbol {
            let shift = bits_per_symbol - 1 - bit;
            let bit_is_one = ((best_idx >> shift) & 1) == 1;
            soft_bits.push(if bit_is_one { confidence } else { -confidence });
        }
    }
    ...
    Ok(DemodResult {
        ...
        freq_offset: estimate_freq_offset(&pcm, AUDIO_SAMPLE_RATE_HZ as f32),
        snr_est,
    })
}
...
fn tone_frequencies(profile: &RadioProfile) -> Result<Vec<f32>, PhyError> {
    ...
    // Practical spacing for the current per-symbol detector. This is looser than
    // strict orthogonal spacing but matches what our implementation can recover.
    let required_step = (profile.symbol_rate.as_hz() as f32 / 2.4).max(250.0);
    let span = required_step * (m as f32 - 1.0);
    let occupied_bw = required_step * m as f32;
    if occupied_bw > CHANNEL_BW_HZ {
        return Err(PhyError::InvalidProfile);
    }

    let start = CARRIER_CENTER_HZ - (span / 2.0);
...
mod tests {
    ...
    fn bfsk_round_trip_soft_bit_sign() {
        ...
        let inv_err = decoded
            ...
            .filter(|(a, b)| (**a ^ 1) != **b)
            .count();
        assert!(err <= 1 || inv_err <= 1, "too many symbol errors: {err}, inv_err: {inv_err}");
    }
}
