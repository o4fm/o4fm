use heapless::Vec;
use o4fm_core::{
    decode_capability_payload, decode_selected_profile_payload, encode_capability_payload,
    encode_selected_profile_payload, FecScheme, Frame, FrameHeader, FrameKind, LinkProfile,
    Modulation, NegotiationProfile, SymbolRate, MAX_NEGOTIATION_PROFILES, MAX_PAYLOAD_BYTES,
};
...
impl LinkMachine {
    ...
    #[must_use]
    pub fn new_with_supported_profiles(profile: LinkProfile, supported: &[NegotiationProfile]) -> Self {
    ...
    pub fn link_tick(&mut self, event: LinkEvent) -> Vec<LinkAction, 8> {
        ...
        match (&self.state, event) {
            ...
            (LinkState::Negotiating, LinkEvent::RxFrame(frame)) if frame.header.kind == FrameKind::Select => {
            ...
            (LinkState::Negotiating, LinkEvent::RxFrame(frame)) if frame.header.kind == FrameKind::Commit => {
            ...
            (LinkState::Data, LinkEvent::RxFrame(frame)) if frame.header.kind == FrameKind::Data => {
...
fn profile_score(profile: NegotiationProfile) -> u32 {
    u32::try_from(profile.modulation.bits_per_symbol()).unwrap_or(1) * profile.symbol_rate.as_hz()
}
